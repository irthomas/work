...
if platform.system() != "Windows":
    ...
    hdf5file_path = os.path.join(paths["BASE_DIRECTORY"], "20210214_114124_0p1e_LNO_1_L_164.h5")




from nomad_ops.core.hdf5.l0p1a_0p1e_to_0p2a.config import NOMAD_TMP_DIR, NA_VALUE, BORESIGHT_VECTOR_TABLE, AREOID_4PPD
#from nomad_ops.core.hdf5.l0p1a_0p1e_to_0p2a.areoid import geoid
from nomad_ops.core.hdf5.l0p1a_0p1e_to_0p2a.areoid_simple import geoid_4ppd
...
#    logger.info("observationType=%s for %s", observationType, hdf5FileIn.file)
    ...
    obsType_in_LO = observationType in ["L", "O"] #day or night limbs - for adding SZA
    ...
    #open areoid model
    with h5py.File(AREOID_4PPD, 'r') as f:
        areoid_ra = f["ra"][...]
    ...
    if obsType_in_DNF or obsType_in_IEGSLO:
            ...
            for rowIndex in range(nSpectra):
                ...
                for pointIndex,fovCorner in enumerate(fovCornersAll[rowIndex]):
                    if not np.isnan(np.min(surfPoints[rowIndex,pointIndex,:,:])):
                        ...
                        if USE_AREOID:
                            areoidRadius[rowIndex,pointIndex,:] = (geoid_4ppd([surfLons[rowIndex,pointIndex,0]], [surfLats[rowIndex,pointIndex,0]], areoid_ra, [True]),
                                    geoid_4ppd([surfLons[rowIndex,pointIndex,1]], [surfLats[rowIndex,pointIndex,1]], areoid_ra, [True]))
        ...
        if obsType_in_IEGSLO:
            ...
            # array([[-0.01026743, -0.15375592,  0.98805551],
            #        [-0.99803073, -0.05957242, -0.01964143],
            #        [ 0.06188084, -0.98631143, -0.15284148]])
            ...
            # array([[ 0.52407424, -0.29567474, -0.79870059],
            #        [ 0.8264444 ,  0.40311806,  0.3930464 ],
            #        [ 0.20575674, -0.86606713,  0.45562253]])
            ...
            #array([  960.66634138,  -339.05210988, -3617.85258172])
            ...
            for i in range(nPoints):
                ...
                surf_indices_s = [np.any(np.isnan(n)) for n in dp[i]["surf_xyz_s"]]
                surf_indices_e = [np.any(np.isnan(n)) for n in dp[i]["surf_xyz_e"]]

                
                #fov vectors in SO frame for each spectrum (changes with bin)
                #array([-0.00442231,  0.00442231,  1.        ])
                dp_tmp[i]["so_fov_vec"] = [np.asfarray(fov[i]) for fov in d_tmp["fov_corners"]]
                
                #convert fov vector to mars reference frame
                #array([ 0.98742096, -0.01549128, -0.15747691])
                ...
                #get nearest point on reduced ellipsoid (xyz) + distance to tangent
                #(array([ -539.17741064,  -309.26950148, -3311.03400533]), 67.0518442700708),
                ...
                #(array([ -539.17741064,  -309.26950148, -3311.03400533])
                ...
                if obsType_in_LO:
                    #vector from TGO to tangent point
                    #array([-1499.84375202,    29.7826084 ,   306.81857638])
                    dp_tmp[i]["tgo2tangent_xyz_s"] = [tan_xyz - obs_xyz for obs_xyz, tan_xyz in zip(d_tmp["obs2mars_mars_spkpos_s"], dp_tmp[i]["tan_pt_rell_xyz_s"])] 
                    dp_tmp[i]["tgo2tangent_xyz_e"] = [tan_xyz - obs_xyz for obs_xyz, tan_xyz in zip(d_tmp["obs2mars_mars_spkpos_e"], dp_tmp[i]["tan_pt_rell_xyz_e"])] 
                    
                    #angle between fov pointing direction and vector from TGO to ellipsoid tangent point
                    #177.49018355175792
                    dp_tmp[i]["fov2tangent_angle_s"] = [sp.vsep(fov_vec_ref, tgo2tangent_xyz) * SP_DPR for fov_vec_ref, tgo2tangent_xyz in zip(dp_tmp[i]["fov_vec_ref_s"], dp_tmp[i]["tgo2tangent_xyz_s"])]
                    dp_tmp[i]["fov2tangent_angle_e"] = [sp.vsep(fov_vec_ref, tgo2tangent_xyz) * SP_DPR for fov_vec_ref, tgo2tangent_xyz in zip(dp_tmp[i]["fov_vec_ref_e"], dp_tmp[i]["tgo2tangent_xyz_e"])]
                
                    #if angle > 90 then tangent point is behind TGO - bad data point
                    limb_indices_s = [True if angle < 90.0 else False for angle in dp_tmp[i]["fov2tangent_angle_s"]]
                    limb_indices_e = [True if angle < 90.0 else False for angle in dp_tmp[i]["fov2tangent_angle_e"]]
                    
                    #combine invalid surface and invalid limb indices
                    indices_s = [True if surf and limb else False for surf, limb in zip(surf_indices_s, limb_indices_s)]
                    indices_e = [True if surf and limb else False for surf, limb in zip(surf_indices_e, limb_indices_e)]
                
                else:
                    indices_s = surf_indices_s
                    indices_e = surf_indices_e

                
                #get dist from reduced ellipsoid to mars centre, lon, lat
                #array([ 3.36887312e+03, -2.62081486e+00, -1.38522674e+00])
                ...
                #get surface solar zenith angle (limb obs only)
                if obsType_in_LO:
                    
                    #first get cartesian xyz from lat/lon of tangent
                    # TRGEPC     0   Target surface point epoch.
                    # SRFVEC     1   Vector from observer to target surface point.
                    # PHASE      2   Phase angle at the surface point.
                    # SOLAR      3   Solar incidence angle at the surface point.
                    # EMISSN     4   Emission angle at the surface point.
                    ...
                    #  1.5934157431802436,
                    #  1.5269917661523629)
                    dp_tmp[i]["surf_illumin_s"] = [sp.ilumin("Ellipsoid", SPICE_TARGET, et, SPICE_PLANET_REFERENCE_FRAME, SPICE_ABERRATION_CORRECTION, SPICE_OBSERVER, xyz) if flag else -999. for flag, et, xyz in zip(indices_s, d["et_s"], dp_tmp[i]["tan_pt_rell_xyz_s"])]
                    dp_tmp[i]["surf_illumin_e"] = [sp.ilumin("Ellipsoid", SPICE_TARGET, et, SPICE_PLANET_REFERENCE_FRAME, SPICE_ABERRATION_CORRECTION, SPICE_OBSERVER, xyz) if flag else -999. for flag, et, xyz in zip(indices_e, d["et_e"], dp_tmp[i]["tan_pt_rell_xyz_e"])]

                    #91.29599709392944
                    dp[i]["sza_s"] = [surf_illumin[3] * SP_DPR if flag else -999. for flag, surf_illumin in zip(indices_s, dp_tmp[i]["surf_illumin_s"])]
                    dp[i]["sza_e"] = [surf_illumin[3] * SP_DPR if flag else -999. for flag, surf_illumin in zip(indices_e, dp_tmp[i]["surf_illumin_e"])]
                ...
                #array([-0.15823115, -0.09076061, -0.98322196])
                ...
                #array([ -549.78710118,  -315.35516761, -3376.96085081])
                ...
                #get slant path distance xyz
                #array([-1510.45344256,    23.69694228,   240.89173091])
                ...
                #array([[ -540.45804103,  -310.00406475, -3318.89822719]])
                ...
                dp_tmp[i]["tan_pt_are_dist_s"] = geoid_4ppd(dp[i]["tan_lon_s"], dp[i]["tan_lat_s"], areoid_ra, indices_s)
                dp_tmp[i]["tan_pt_are_dist_e"] = geoid_4ppd(dp[i]["tan_lon_e"], dp[i]["tan_lat_e"], areoid_ra, indices_e)
            ...
            d["tilt_angle_e"] = [sp.vsep(ul2ll_fov_vec_ref, mars2centre_fov_vec_ref) * SP_DPR if flag else -999. for flag, ul2ll_fov_vec_ref, mars2centre_fov_vec_ref in zip(indices_e, d_tmp["fovTopBottomUnitVectors_e"], d_tmp["marsCentre2fovCentreUnitVector_e"])]
    ...
    if obsType_in_IEGSLO:
        ...
        #     # dataset_mappings["Geometry/Point%s/FOVWeight" %i] = {"units":"NO UNITS", "desc":"", "dtype":np.float, "var":dp[i][""]}
            ...
            if obsType_in_LO:
                dataset_mappings["Geometry/Point%s/SunSZA" %i] = {"units":"DEGREES", "desc":"Solar zenith angle of surface below tangent point", "dtype":np.float, "var":dp[i]["sza"]}
