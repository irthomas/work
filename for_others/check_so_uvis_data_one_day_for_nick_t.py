...
GET SO DATA FOR 21ST JANUARY FOR NICK THOMAS
"""
...
# regex = re.compile("20240121_......_.*_SO_.*")
# regex2 = re.compile("20240121_......_.*_UVIS_[IE]")

regex = re.compile("20200926_031944_.*_SO_.*")
regex2 = re.compile("20200926_031944_.*_UVIS_[IE]")
...
def get_so_data(regex, bin_ix):
    ...
    h5_fs, h5s, _ = make_filelist(regex, file_level, path=r"C:\Users\iant\Documents\DATA\hdf5")

    d = {}
    for file_ix, (h5, h5_f) in enumerate(zip(h5s, h5_fs)):

        print(file_ix, len(h5s))
...
with PdfPages("solar_occultation_%s.pdf" % name) as pdf:
    ...
    for h5_prefix in d.keys():
        ...
        if "Fullscan" not in h5_prefix:
            ...
            fig, axes = plt.subplots(ncols=n_cols, figsize=(20, 8), sharey=True, constrained_layout=True, rasterized=True)
            for i, h5 in enumerate(d[h5_prefix].keys()):
                ...
                axes[i].set_title("Order %i" % orders[0])
                axes[i].grid()
