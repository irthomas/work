...
#    logger.info("observationType=%s for %s", observationType, hdf5FileIn.file)
    if observationType is None:
        raise RuntimeError("Observation type is not defined")
    ...
    obsType_in_IESMUL = observationType in ["I","E","G","S","L"] #limb measurements are like occultations
    ...
    if obsType_in_DNFV or obsType_in_IESMUL:
...
#            logger.info("Adding nadir geometry for observation of type %s in file %s", observationType, hdf5file_path)
        ...
        if USE_REDUCED_ELLIPSE:
            bodyAxes = marsAxes*(marsAxes[2]-8.)/marsAxes[2]
        else:
            bodyAxes = marsAxes
