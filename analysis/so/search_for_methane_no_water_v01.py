...
from retrievals.SO_retrieval_v02b import NOMAD_ret, Rodgers_OEM, forward_model
...
#regex = re.compile("20191224_235111.*_SO_A_[IE]_136")
...
for spectrumIndex in range(len(tangent_alt_binned)):
    ...
    binnedCentreAltitude = binnedCentreAltitudes[spectrumIndex]
    matchingIndex = (np.abs(obsDict["alt"] - binnedCentreAltitude)).argmin()
    print("Altitude=%0.3f, error=%0.3f" %(binnedCentreAltitude, binnedCentreAltitude - obsDict["alt"][matchingIndex]))
    ...
    while np.abs(relative_difference) > 0.05:
        ...
        if np.abs(relative_difference) < 0.05:
            print("%i/%i found" %(spectrumIndex, len(binnedCentreAltitudes)))
