...
"""for testing/defining baseline points"""
# regex = re.compile("202307.._......_.*_SO_A_[IE]_%s" %order)
# PLOT = True
...
def get_altitude(x, y, alts, order, trans, plot=False):
    ...
    if plot:
        plt.plot(alts, polyvals_det_centre)
    
    smoothed = savgol_filter(polyvals_det_centre, 19, 2)
    if plot:
        plt.plot(alts, smoothed)
    ...
    if below_ix > 0:
        ...
        if plot:
            plt.scatter(alts[above_ix], smoothed[above_ix])
            plt.scatter(alts[below_ix], smoothed[below_ix])
            plt.scatter(alt, CHOSEN_TRANS)
...
def make_trans_dict(regex, file_level):
    ...
    for h5 in progress_bar(h5s):
        ...
        if order not in baseline_points.keys():
            ...
            alt, ix = get_altitude(x, y, alts, order, CHOSEN_TRANS, plot=PLOT)
...
if len(combined_orders["orders"]) == 0:
    ...
    order = "_".join(["%s" %i for i in orders])
# stop()
...
#     if SAVE_PLOTS:
#         plt.savefig("so_order_%i_my%0.0f.png" %(order, my))
...
if plot_level < 2:
    if len(mys) > 0:
        ...
        if SAVE_PLOTS:
            plt.savefig("so_order_%s_all_mys.png" %(order))
        ...
        cbar.set_label("Tangent altitude above areoid (km)")
        if SAVE_PLOTS:
            plt.savefig("so_order_%s_all_mys_overlap.png" %(order))
...
# train_ixs = np.where((out_d["my"] == 35))[0]
train_ixs = np.where((out_d["my"] == 36))[0]
# train_ixs = np.where((out_d["my"] > 33))[0]
...
# lat_ixs = np.where((np.abs(diff) < diff_std*2.0))[0]
# np.savetxt("good_ixs_order_%s_%s.txt" %(order, "_".join(ML_VARIABLES)), lat_ixs, fmt="%i")


if plot_level < 3:
    plt.figure(figsize=(10,5), constrained_layout=True)
    plt.title("Predicted altitudes for inputs %s for order %s" %(", ".join(ML_VARIABLES), order))
    plt.xlabel("Ls")
    plt.ylabel("Latitude")
    plt.grid()
    ...
    if SAVE_PLOTS:
        plt.savefig("so_order_%s_all_mys_predicted.png" %(order))
    ...
    cbar.set_label("Tangent altitude above areoid (km)")
    if SAVE_PLOTS:
        plt.savefig("so_order_%s_%s_all_mys_predicted_overlap.png" %(order, "_".join(ML_VARIABLES)))
...
if plot_level < 4:
    ...
    plt.figure(figsize=(10,5), constrained_layout=True)
    plt.title("Measured minus predicted altitudes for inputs %s for order %s" %(", ".join(ML_VARIABLES), order))
    plt.xlabel("Ls")
    plt.ylabel("Latitude")
    plt.grid()
    ...
    if SAVE_PLOTS:
        plt.savefig("so_order_%s_%s_all_mys_diff.png" %(order, "_".join(ML_VARIABLES)))
    ...
    if SAVE_PLOTS:
        plt.savefig("so_order_%s_%s_all_mys_diff_overlap.png" %(order, "_".join(ML_VARIABLES)))
    ...
    # plt.savefig("so_order_%s_all_mys_diff.png" %(order))
    ...
    if SAVE_PLOTS:
        plt.savefig("so_order_%s_%s_all_mys_diff_lat_bin.png" %(order, "_".join(ML_VARIABLES)))
...
#model MY35 and try to pred MY36 and MY34
