...
for reduction in reductions:
    ...
    if reduction == 0.0:
        ...
        ax1a.text(current_dt + timedelta(days=30), hours_per_day + 0.25,
                  "Mean operating hours over past %i days = %0.2f hours" % (N_DAYS_PROJECT_BACK, hours_per_day))
...
for channel in channels:
    ...
    # load data only if not in memory
    if channel == "SO":
        if "dates_so" not in globals() or get_data:
            dates_so = get_daily_usage(channel, regex, file_level)
    if channel == "LNO":
        if "dates_lno" not in globals() or get_data:
            dates_lno = get_daily_usage(channel, regex, file_level)
