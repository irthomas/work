...
for h5_prefix, solar_line_data_all in solar_line_dict.items(): #loop through files
    ...
    for i in progress(range(arr.shape[1])):
        ...
        result = smodel.fit(y, x=x, a=sinc_apriori["a"], b=sinc_apriori["b"], c=sinc_apriori["c"], d=sinc_apriori["d"], e=sinc_apriori["e"], f=sinc_apriori["f"])
    ...
    for i in progress(range(arr2.shape[1])):
        ...
        smodel = Model(sinefunction)
        result = smodel.fit(y, x=x, nan_policy="omit", a=sinc_apriori["a"], b=sinc_apriori["b"], c=sinc_apriori["c"], d=sinc_apriori["d"], e=sinc_apriori["e"], f=sinc_apriori["f"])
