...
# regex = re.compile("20180716.*_%s_.*_CM" % channel)  # search specific file SO
...
if __name__ == "__main__":
    if file_level == "hdf5_level_1p0a":
        if list_files:
            h5_filenames, h5_prefixes = list_miniscan_data_1p0a(regex, starting_orders, aotf_steppings, binnings, path=None)
            if "d" not in globals():
    ...
    for h5_prefix in progress(h5_prefixes):
        ...
        if OUTPUT_FILE_TYPE == "h5":
            ...
            hdul.writeto(os.path.join(MINISCAN_PATH, channel, "%s.fits" % h5_prefix), overwrite=True)
