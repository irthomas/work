...
PLOT GROUNDTRACKS FOR CHOSEN BINS ON PHOBOS ILLUMINATION MAP FOR START AND END OF OBSERVATION (OR AVERAGE OF BOTH)
GETS GEOMETRY OF MANY POINTS AROUND EDGE OF CHOSEN BINS TO GIVE GOOD GROUNDTRACK

IF OFF-POINTING: GET VECTOR TO PHOBOS AND CALCULATE EXPECTED FOV

CHECK THAT THE SPICE KERNEL MK CONTAINS THE PHOBOS DSK
...
def make_obs_dict(observationDatetimes):
    ...
    d_tmp["obs_subpnt_s"] = [sp.subpnt(SPICE_INTERCEPT_METHOD, SPICE_TARGET, et, SPICE_PLANET_REFERENCE_FRAME,
                                       SPICE_ABERRATION_CORRECTION, SPICE_OBSERVER) for et in d["et_s"]]
    # d_tmp["obs_subpnt_e"] =
    # [sp.subpnt(SPICE_INTERCEPT_METHOD,SPICE_TARGET,et,SPICE_PLANET_REFERENCE_FRAME,SPICE_ABERRATION_CORRECTION,SPICE_OBSERVER) for et in d["et_e"]]
    ...
    d_tmp["sun_subpnt_s"] = [sp.subpnt(SPICE_INTERCEPT_METHOD, SPICE_TARGET, et, SPICE_PLANET_REFERENCE_FRAME,
                                       SPICE_ABERRATION_CORRECTION, "SUN") for et in d["et_s"]]
        ...
        for et in ets:
            ...
            try:
                sincpt = sp.sincpt(SPICE_SHAPE_MODEL_METHOD, SPICE_TARGET, et, SPICE_PLANET_REFERENCE_FRAME,
                                   SPICE_ABERRATION_CORRECTION, SPICE_OBSERVER, DREF, fov)[0]
            except sp.stypes.NotFoundError:
                sincpt = (np.zeros(3) + NA_VALUE, NA_VALUE, np.zeros(3) + NA_VALUE)[0]

            if not np.isnan(sincpt[1]):  # != NA_VALUE:
                ilumin = sp.ilumin(SPICE_SHAPE_MODEL_METHOD, SPICE_TARGET, et, SPICE_PLANET_REFERENCE_FRAME,
                                   SPICE_ABERRATION_CORRECTION, SPICE_OBSERVER, sincpt)
...
if __name__ == "__main__":
    # h5s = ["20220710_200313_0p3a_LNO_1_P_148"]
    # h5 = "20220713_164911_0p3a_LNO_1_P_148"]
    # h5 = "20230225_220121_0p1a_LNO_1"]
    # h5 = "20250628_083013_0p1a_LNO_1"]
    # h5 = "20250628_083013_0p1a_LNO_1"]
    # h5 = "20240126_123236_0p3a_LNO_1_P_176"]
    h5s = []
    ...
    for h5 in h5s:
        ...
        for bin_number in bin_numbers:
            ...
            # define left/right/top/bottom of row FOV in arcminutes
            min_x = -2.
            max_x = 2.
            min_y = detector_offset
            max_y = detector_offset + bin_height

            # make vectors to many points around edge of the FOV
            ...
            et = d["et_s"][int(len(d["et_s"])/2)]  # choose a time within the observation
            ...
            illum_indices = []
            if "groundtrack_illum_start" in PLOT_TYPES:
                illum_indices.append(0)
            if "groundtrack_illum_end" in PLOT_TYPES:
                illum_indices.append(-1)

            if len(illum_indices) > 0:
            ...
            for frame_index in illum_indices:
                ...
                cbar2.set_label("Solar zenith angle (degrees)", rotation=270, labelpad=15)

                ax2.legend()
                fig2.savefig("phobos_sza_%s_bin_%i_%s.png" % (h5, bin_number, {0: "start", -1: "end"}[frame_index]))

            illum_indices = []
            if "groundtrack_illum_combined" in PLOT_TYPES:
                illum_indices = [0, -1]
                ...
                for frame_index in illum_indices:
                    ...
                    for lon_ix, lon in enumerate(lons):
                        for lat_ix, lat in enumerate(lats):
                            ...
                            ilumin = sp.ilumin(SPICE_SHAPE_MODEL_METHOD, SPICE_TARGET, et, SPICE_PLANET_REFERENCE_FRAME,
                                               SPICE_ABERRATION_CORRECTION, SPICE_OBSERVER, surf_xyzs[index])
                            # add instead of assign
                            phobos_illumination[len(lats)-1-lat_ix, lon_ix] += ilumin[3] * SP_DPR

                # divide by 2 to give average
                phobos_illumination /= 2.0
        ...
        #             ilumin = sp.ilumin(SPICE_SHAPE_MODEL_METHOD, SPICE_TARGET, et, SPICE_PLANET_REFERENCE_FRAME,
        #                                SPICE_ABERRATION_CORRECTION, SPICE_OBSERVER, surf_xyz)
