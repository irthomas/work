...
ANALYSE PHOBOS FULLSCAN AND NON-FULLSCAN LEVEL 0.1A (AND 0.3A) FILES
"""
...
"""observation name:{"h5":hdf5 filename, "good_indices":frames with best signal and least noise (remove for all), "good_row_indices":bins with highest signal, "orders_crism":orders to fit to CRISM spectra}"""
obs_types = {
    "Gap 2 orders 1 60s": {"h5": "20250628_083013_0p1a_LNO_1", "good_indices": [*range(0, 121)], "good_row_indices": [0, 1], "orders_crism": [160, 168, 170]},


    # "Gap 2 orders 1 60s": {"h5": "20230713_092806_0p1a_LNO_1", "good_indices": [*range(21, 96)], "good_row_indices": [0, 1, 2], "orders_crism": [160, 168, 170]},
...
for obs_type in obs_types.keys():
    ...
    # correct bad pixels
    if bad_pixel_type == "dict":
        for i, unique_bin in enumerate(unique_bins):
            bad_pixels = bad_pixel_d[(unique_bin, binning)]
            y_3d[:, i, bad_pixels] = np.nan
    # elif bad_pixel_type == "iteration":

    #     """correct bad pixels with a polyfit"""
    #     x = np.arange(y_3d.shape[2])

    #     # loop through 15 times, fitting and removing worst pixels
    #     n_loops = 15
    #     for loop in range(n_loops):
    #         for row in range(y_3d.shape[1]):
    #             for frame in range(y_3d.shape[0]):
    #                 y = y_3d[frame, row, :]
    ...
    #     fitted_spectra, corr_spectra, fitted_params = fit_spectra(y_3d, solar_spectra[phobos_unique_order])

    #     # y_spectral_mean = np.nanmean(y_3d[:, :, :], axis=2)  # spectral mean
    ...
    if offset_correction == "first_bin":
        y_spectral_mean = np.nanmean(y_3d[:, :, 100:300], axis=2)  # spectral mean
        y_spectral_mean -= np.tile(y_spectral_mean[:, 0], (len(unique_bins), 1)).T  # subtract the last bin
    # subtract the last bin?
    elif offset_correction == "last_bin":
        y_spectral_mean = np.nanmean(y_3d[:, :, 100:300], axis=2)  # spectral mean
        y_spectral_mean -= np.tile(y_spectral_mean[:, -1], (len(unique_bins), 1)).T  # subtract the last bin
    # nothing, just bin
    else:
        y_spectral_mean = np.nanmean(y_3d[:, :, 100:300], axis=2)  # spectral mean
    ...
    if good_indices == "":
        good_indices = np.arange(y_3d.shape[0])
    ...
    if plot_level < 0:
        ...
        for bin_ in range(y_3d.shape[1]):
            ...
            for i, y_spectrum in enumerate(y_3d[good_indices, bin_, :]):
    ...
    if plot_level < 2:
        ...
        for order in cal_d.keys():
            plt.plot(cal_d[order]["y_spectrum"], color=colour_order_dict[order]["colour"], label="Diffraction order %i" % order)
            plt.plot([0, len(cal_d[order]["y_spectrum"])-1], [cal_d[order]["y_centre_mean"], cal_d[order]["y_centre_mean"]],
                     color=colour_order_dict[order]["colour"], linestyle=":", label="Order %i mean signal" % order)
        plt.legend()
    ...
    for order in unique_orders:
        ...
        counts_d[order] = {"indices": indices, "y_column_mean": y_column_mean_good,
                           "y_column_solar_scaled": y_column_solar_scaled, "colour": colour_order_dict[order]["colour"]}
    ...
    if plot_level < 2.5:
        ...
        for axes_ix, bin_ix in enumerate(good_row_indices):
            for order in unique_orders:
                ...
                axes[axes_ix].scatter(counts_d[order]["indices"], counts_d[order]["y_spectral_mean"][:, bin_ix], color=colour, label="Order %i" % order)
                axes[axes_ix].plot([np.min(counts_d[order]["indices"]), np.max(counts_d[order]["indices"])],
                                   [np.mean(counts_d[order]["y_spectral_mean"][:, bin_ix]), np.mean(counts_d[order]["y_spectral_mean"][:, bin_ix])],
                                   color=colour, linestyle=linestyle)
            ...
            axes[axes_ix].grid()
        fig.subplots_adjust(bottom=0.15)
        plt.savefig("lno_phobos_signal_each_bin.png")

    if plot_level < 3:
        # plot spectrally averaged signal for all bins averaged
        plt.figure(figsize=(10, 5))
        plt.title("LNO spectrally binned signal for each diffraction order")
        ...
        plt.grid()
        plt.savefig("lno_phobos_signal_sum_good_bins.png")
    ...
    if plot_level < 4:
        ...
        axes1[2].plot(unique_orders, [counts_d[order]["y_all_column_solar_scaled_mean"]
                      for order in unique_orders], color="k", label="Y counts mean scaled to solar")
        ...
        plt.savefig("lno_phobos_solar_corr.png")
