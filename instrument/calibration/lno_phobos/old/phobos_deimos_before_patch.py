...
PLOT RAW SIGNAL FOR LNO+UVIS AND APPROXIMATE HR SPECTRUM AND SNR ESTIMATION
NO COMPARISON OF ORDERS OR CALIBRATION - BEST IF JUST ONE ORDER MEASURED PER OBS
"""
...
for i, filename in enumerate(file_info.keys()):
    ...
    # y_all in 0.3a data is 2 dimensional- make 3d
    if "LNO" in filename:
    ...
    elif "UVIS" in filename:
        y_all_3d = y_all
        unique_bins = np.arange(y_all.shape[1])

    print(filename, y_all_3d.shape)

    y_mean_px = np.mean(y_all_3d[frame_range[0]:frame_range[1], :, px_range], axis=2).T  # mean of spectral pixels
    ...
    if "LNO" in filename:
        ...
        y_mean_px -= np.tile(np.mean(y_mean_px[:, -20:], axis=1), (len(unique_bins), 1)).T  # subtract the last bin

    # remove anomalies
    std_cutoff = 3.0

    mean = np.mean(y_mean_px)
    std = np.std(y_mean_px)
    ...
    ax[i][1].set_title("%s: spectrally averaged counts (columns %i-%i)\n%s - %s"
                       % (filename, min(px_range), max(px_range), obs_times[0], obs_times[1]))
    im = ax[i][0].imshow(y_mean_px.T, aspect=aspect)
    ...
    if "LNO" in filename:
        ...
        # find row with highest signal
        mean = np.mean(y_all_3d[frame_range[0]:frame_range[1], :, px_range], axis=(0, 2))
        mean[np.isnan(mean)] = 0.
        max_row = np.argmax(mean)

        # illuminated row
        det_region = y_all_3d[frame_range[0]:frame_range[1], max_row, :]
        spectrum = np.mean(det_region, axis=0)

        # dark row
        dark_stds = []
        for row in range(y_all_3d.shape[1]):
            if row not in [max_row-1, max_row, max_row+1]:
                det_region = y_all_3d[frame_range[0]:frame_range[1], row, :]
                dark = np.mean(det_region, axis=0)

                if row == y_all_3d.shape[1]-1:
                    dark_to_plot = dark
