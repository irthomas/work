...
"""search the json made by script phobos_auto_rad_cal.py
orders_to_find = find observations containing these order combinations
nominal_orders = interpolate these orders to crism data
orders_to_scale = orders to include when scaling curves to match each other (to ignore noisiest data points)
"""
...
for name in selected_orders.keys():
    ...
    for h5 in phobos_d.keys():
        ...
        if list(orders) in selected_orders[name]["orders_to_find"]:
            ...
            # get corrected signal in counts and interpolate to the values at the nominal orders
            # doesn't do anything if observation orders = nominal orders
            norm_vals = phobos_d[h5]["norm_vals"]
            norm_vals_norm = norm_vals/np.max(np.mean(norm_vals[:]))
            ...
            # plot all values scaled to red and blue CRISM
            red_scaled = phobos_d[h5]["red_scaled"]
            blue_scaled = phobos_d[h5]["blue_scaled"]
            ax2a.scatter(nominal_um, red_scaled, color="red", alpha=0.2)
            ax2a.plot(nominal_um, red_scaled, color="red", alpha=0.2)
            ax2a.scatter(nominal_um, blue_scaled, color="blue", alpha=0.2)
            ax2a.plot(nominal_um, blue_scaled, color="blue", alpha=0.2)
    ...
    snrs = interp_mean/interp_std
    print("SNRs:", snrs)

    # find indices for orders to scale mean values for each observed order to CRISM
