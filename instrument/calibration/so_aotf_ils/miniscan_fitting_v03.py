...
from instrument.calibration.so_aotf_ils.simulation_functions import get_start_params, make_param_dict, calc_spectrum, fit_spectrum, fit_aotf, F_aotf2
...
regex = re.compile("20190416_020948_0p2a_SO_1_C") #(approx. orders 194-195) in steps of 1kHz
# regex = re.compile("20210201_111011_0p2a_SO_2_C") #(approx. orders 188-202) in steps of 8kHz
...
# indices = range(0, 100, 1) #for 20190416_020948_0p2a_SO_1_C
...
variables_fit = {"A":[], "A_nu0":[], "solar_line_area":[], "chisq":[], "temperature":[]}
...
good_indices = np.where(variables_fit["error"] < 0.01)[0]
...
np.savetxt("%s_aotf_function.txt" %hdf5_filename, np.array([x2,x,t,y]).T, fmt="%.6f", delimiter=",", header="Frequency,Wavenumber,Temperature,AOTF function")
