# -*- coding: utf-8 -*-
"""
Created on Fri Apr 30 20:01:09 2021
...
for file_index, (hdf5_file, hdf5_filename) in enumerate(zip(hdf5Files, hdf5Filenames)):
    ...
    for i, key in enumerate(d["y"].keys()):
        if i == 0:
            ...
            px_values = np.asfarray(d["y"][key])[:, 130]
            ...
            plt.figure()
            plt.plot(unique_aotf_freqs, px_values)
            plt.plot(unique_aotf_freqs, fit_aotf_wave)
            
            plt.figure()
            plt.plot(unique_aotf_freqs, values_aotf_wave)
