...
ACS_DATA_PATH_ROOT = r"D:\DATA\psa\ACS"
...
for xml_path in xml_paths[0:1]:
    ...
    if channel in ["mir"]:
        ...
        if log:
            write_log("%s %i %i %i %i %i" % (xml_filename, data.shape[0], geometry1.shape[0],
                      geometry2.shape[0], geometry3.shape[0], data.shape[0]-geometry1.shape[0]))
