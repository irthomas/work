...
def hapi_fetch(molecule, nu_start, nu_end):
    ...
    if molecule in ["H2O"]:
        ...
        hapi.fetch_by_ids(table_name, iso_codes, nu_start, nu_end, ParameterGroups=['Voigt_CO2'])
...
def get_abs_coeff(molecule, nu_range, nu_step, mol_ppmv, co2_ppmv, t, pressure, clear=False):
    ...
    if molecule == "CO2":
        Components=[
            (component, 1, mol_ppmv/co2_ppmv*hapi.abundance(component, 1)),
            (component, 2, mol_ppmv/co2_ppmv*hapi.abundance(component, 2)),
            (component, 3, mol_ppmv/co2_ppmv*hapi.abundance(component, 3)),
        ]

    else:
        Components=[
            (component, 1, mol_ppmv/co2_ppmv*hapi.abundance(component, 1)*scalar),
            (component, 2, mol_ppmv/co2_ppmv*hapi.abundance(component, 2)*scalar),
            (component, 3, mol_ppmv/co2_ppmv*hapi.abundance(component, 3)*scalar),
            (component, 4, mol_ppmv/co2_ppmv*hapi.abundance(component, 4)*scalar),
            (component, 5, mol_ppmv/co2_ppmv*hapi.abundance(component, 5)*scalar),
