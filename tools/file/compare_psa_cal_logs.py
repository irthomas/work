...
if platform.system() == "Windows":
    LOG_FILE_DIR = os.path.join(BASE_DIRECTORY, "psa_cal")
else:
    LOG_FILE_DIR = os.path.join(DATASTORE_ROOT_DIRECTORY, "nomadlsy/datastore/pds/logs/psa_cal/")
...
with open(os.path.join(BASE_DIRECTORY, "psa_cal_log_ingested.txt"), "w") as f:
    for xmlFilename in ingestedXmlFilenameList:
        f.write("%s\n" %xmlFilename)

with open(os.path.join(BASE_DIRECTORY, "psa_cal_log_local.txt"), "w") as f:
...
with open(os.path.join(BASE_DIRECTORY, "psa_cal_log_mismatches.txt"), "w") as f:
...
if not error:
    ...
    transferFileList = localNotInPsaPath[0:3]
