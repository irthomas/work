...
def scan_exm_packets(file_path):
    ...
    while i < len(hex_in):
        ...
        il = 1
        tm["type"] = int.from_bytes(hex_in[i:i+il])
        i += il

        il = 3
        tm["size"] = int.from_bytes(hex_in[i:i+il])
        i += il
        ...
        if tm["type"] in [37, 60]:
            ...
            il = 8
            tm["ts"] = int.from_bytes(hex_in[i:i+il])
            i += il

            il = 2
            tm["count"] = int.from_bytes(hex_in[i:i+il])
            i += il
        ...
        il = 2
        tm["cs"] = int.from_bytes(hex_in[i:i+il])
        i += il
...
def reconstruct_frames(tms):
    ...
    big_arr = np.zeros((N_ROWS, N_COLS), dtype=int)
    for i in range(1850, 1880):
        if tms[i]["type"] == 28:
    ...
    #         # struct.unpack(tms[i]["data"])
            ...
            for ib in range(niters):
                ...
                if BYTES_PER_VALUE == 2.5:
                    bs = ib * 5
                    b5 = int.from_bytes(struct.unpack('>5s', byte_string[bs:bs+5])[0])
