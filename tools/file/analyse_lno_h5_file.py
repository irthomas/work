# -*- coding: utf-8 -*-
"""
Created on Fri Apr  5 09:34:38 2024
...
Y_FIELDS = {
    ("1p0a", "LNO", "DP"): "YReflectanceFactor",
    ("1p0a", "LNO", "DF"): "YReflectanceFactor",
}


h5 = "20180327_052851_1p0a_LNO_1_DP_168"
h5_split = h5.split("_")
...
if channel == "LNO" and obs_type in ["DF", "DP"]:
    ...
    for key, value in hsk_d.items():
        if key not in ["DateTime", "MOTOR_POWER_DAC_CODE_LNO"]:
            ax2.plot(value[1:], label=key)

    ax2.legend()
    ax2.grid()

    for inst_param in ["BackgroundSubtraction", "Binning", "IntegrationTime", "NumberOfAccumulations", "WindowHeight", "WindowTop"]:
